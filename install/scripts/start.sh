...
notify-send "Looking for Docker data medium" -t 25000
...
if [ $? -eq 0 ]
then 
  notify-send  "Docker data medium found"  -t 20000
  /tmp/startapp_inv.sh > /tmp/startapp_inv.log 2>&1
else
  notify-send "Docker data medium not found. Connect a medium (see doc) and run 'sudo /usr/local/bin/start.sh -d /dev/<my-docker-drive>'" --timeout 50000
fi
